...
if __name__ == "__main__":
    ...
    # GENERIC twins loader (retargeted from MedHallu): DATA_FILE = JSONL, one twin/line with keys
    # {qid, Q, K, truthful, halluc, difficulty?, category?, source?}. Scoring below is dataset-agnostic
    # and IDENTICAL to score_strong.py (judge / P(True) / ref-NLI). H=0=truthful, H=1=hallucinated.
    import json as _json
    DATA_FILE = os.environ["DATA_FILE"]
    log(f"loading generic twins from {DATA_FILE} (N_Q={N_Q}); model={MODEL}; K_SE={K_SE}; SKIP_SE={SKIP_SE}")
    def as_text(x): return "\n".join(str(t) for t in x) if isinstance(x, list) else str(x)
    twins = [_json.loads(l) for l in open(DATA_FILE) if l.strip()]
    if N_Q < len(twins): twins = twins[:N_Q]
    rows = []
    for r in twins:
        Q = str(r["Q"]).strip(); K = as_text(r.get("K", "")).strip()
        diff = str(r.get("difficulty", "")).strip().lower(); cat = str(r.get("category", "")).strip()
        q = int(r["qid"]); src = str(r.get("source", "generic"))
        rows.append(dict(idx=len(rows), qid=q, H=0, difficulty=diff, category="",  source=src, answer=str(r["truthful"]).strip(), Q=Q, K=K))
        rows.append(dict(idx=len(rows), qid=q, H=1, difficulty=diff, category=cat, source=src, answer=str(r["halluc"]).strip(), Q=Q, K=K))
    ...
    def nli_probs(prem, hyp, bs=128, max_len=512):
        ...
        for i in range(0, len(prem), bs):
            enc = ntok(prem[i:i+bs], hyp[i:i+bs], truncation="longest_first", max_length=max_len, padding=True, return_tensors="pt").to(nli_dev)
    ...
    META_OUT = OUT.replace(".csv", "_meta.json")   # per-dataset, derived from OUT (never clobber the MedHallu meta)
    json.dump({"n": len(rows), "n_q": len(uq), "K_SE": K_SE, "model": MODEL, "nli": NLI_MODEL,
               "scores": names, "auroc": summ, "seed": SEED, "data_file": os.environ.get("DATA_FILE", "")},
